...
const SpecularButton = ({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "relative m-0 inline-flex cursor-pointer items-center justify-center border-none font-medium leading-none tracking-[0.01em] outline-none transition-transform duration-150 active:scale-[0.97] disabled:cursor-default disabled:opacity-55 disabled:active:scale-100 text-(--sb-text-color) rounded-(--sb-radius) bg-[color-mix(in_srgb,var(--sb-tint)_calc(var(--sb-tint-opacity)*100%),transparent)] backdrop-blur-(--sb-blur) shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_8px_24px_rgba(0,0,0,0.25)] focus-visible:outline-2 focus-visible:outline-offset-[3px]",
        SIZES[size] || SIZES.md,
        className
      )}
      style={
